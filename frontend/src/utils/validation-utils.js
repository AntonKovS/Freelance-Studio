export class ValidationUtils {      //как и во всех утилитках мы не будем создавать экземпляр класса, поэтому сделаем статический метод
    ...
    static validateField(element, options) {
        ...
        if (options) {
            if (options.hasOwnProperty('pattern')) {        //что в таком объекте есть свойство pattern
