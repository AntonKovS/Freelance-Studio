...
export class Dashboard {
    ...
    async getOrders() {
        ...
        this.loadCalendarInfo(response.orders);  //будет выполняться после предыдущей, так как та не асинхронная
    }
    loadOrdersInfo(orders) {
        ...
        document.getElementById('in-progress-orders').innerText = orders.filter(order => [config.orderStatuses.new, config.orderStatuses.confirmed].includes(order.status)).length;
                //это условие вместо... = orders.filter(order => order.status === config.orderStatuses.new || order.status === config.orderStatuses.confirmed).length;
