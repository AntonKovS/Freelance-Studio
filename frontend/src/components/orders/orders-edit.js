...
export class OrdersEdit {
    ...
    async updateOrder(e) {
        e.preventDefault();     //чтобы останавливать отправку формы, если она будет отправляться
        ...
        if (ValidationUtils.validateForm(this.validations)) {
            ...
            if (this.completeDate || this.completeDate === false) {
                changedData.completeDate = this.completeDate ? this.completeDate.toISOString() : null;     //используем toISOString() (формат даты ISO8601), так как это moment.js обект (также как делали на orders-create.js 179 строка)
            }
            if (this.deadlineDate) {
                changedData.deadlineDate = this.deadlineDate.toISOString();     //используем toISOString() (формат даты ISO8601), так как это moment.js обект (также как делали на orders-create.js 179 строка)
            }
            if (this.scheduledDate) {
                changedData.scheduledDate = this.scheduledDate.toISOString();     //используем toISOString() (формат даты ISO8601), так как это moment.js обект (также как делали на orders-create.js 179 строка)
            }
