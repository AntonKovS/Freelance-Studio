...
export class Router {
    constructor() {
        ...
        this.routes = [
            {
                route: '/',      //будем делать без # во всем проекте
                title: 'Дашборд',
                filePathTemplate: '/templates/pages/dashboard.html',       //т.к. используем fetch и запрос происходит с главной папки проекта, а в результате сборки мы файлы разместим в папке templates
                useLayout: '/templates/layout.html',
                load: () => {
                    new Dashboard(this.openNewRoute.bind(this));    //определяем какой файл js использовать и сделать экземпляр класса из него
                },
                scripts: [
                    'moment.min.js',
                    'moment-ru-locale.js',
            ...
            },
            {
                ...
                load: () => {
                    ...
                    new Login(this.openNewRoute.bind(this));   //делаем bind, чтобы сохранить контекст класса Router
                },
                unload: () => {
    ...
    initEvents() {
        window.addEventListener('DOMContentLoaded', this.activateRoute.bind(this));     //не вызываем здесь функцию, без (), если не сделать bind, то в функцию будет привязан контекст события (контент страницы загрузился (по факту 1й раз пользователь открыл приложение или перезагрузка страницы), чтобы отловить login, sing up итп).
        window.addEventListener('popstate', this.activateRoute.bind(this));         //событие, когда сделали что-то на странице, перешли на другую страницу, т.е. когда изменился url (пользователь перешел на другую страницу в рамках работающего приложения)
    ...
    async activateRoute(e, oldRoute = null) {
        ...
        if (newRoute) {
            ...
            if (newRoute.filePathTemplate) {        // document.body.className = '';  //не будем так делать, так как удаляем вообще все классы, даже если библиотеки их добавили. А сделаем unload.
                ...
                if (newRoute.useLayout) {
                    ...
                    this.activateMenuItem(newRoute);    //используем только когда есть layout, когда нет, то пользователь и не видит этого блока
                } else {
